  ...
  return (
    <div className="flex h-full flex-col border-r border-gray-200 bg-white">
      <div className="flex-shrink-0 border-b border-gray-200 px-5 py-3">
        ...
        <p className="mt-0.5 text-[11px] text-gray-300">드래그하여 Do에 배치 · 클릭으로 시간 지정</p>
      </div>

      <div className="flex-1 overflow-y-auto p-2.5">
        {items.map((item, idx) => {
          const color = COLORS[idx % COLORS.length];
          const eventData = JSON.stringify({ title: item.text, duration: "01:00:00", color });
          return (
            <div
              key={item.id}
              draggable={!item.done}
              ...
              className={`group mb-1.5 flex items-center gap-2.5 rounded-lg bg-gray-50 px-3.5 py-2.5 transition-colors ${
                draggingId === item.id ? "opacity-40" : ""
              } ${!item.done ? "cursor-grab active:cursor-grabbing" : ""}`}
            >
              <button
